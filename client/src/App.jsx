...
const TeammatePlaceholder = ({ name }) => (
  <div className="min-h-[70vh] flex items-center justify-center p-6 text-center">
    <div className="glass-panel p-8 rounded-3xl max-w-md border border-white/5 shadow-2xl relative overflow-hidden">
      ...
      <h2 className="font-sora text-2xl font-bold text-white mb-2">{name}</h2>
  ...
  return (
    <Router>
      <div className="min-h-screen flex flex-col bg-darkBg text-slate-100 font-sans selection:bg-darkPrimary/30 selection:text-white">
        ...
        {/* Navigation Header */}
        <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'glass-panel py-4 shadow-lg border-b border-white/5' : 'bg-transparent py-6'}`}>
            ...
            <nav className="hidden xl:flex items-center gap-6">
              {navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  to={link.href} 
                  className="text-xs font-semibold text-gray-400 hover:text-white transition-colors duration-200"
                >
                  {link.name}
                </Link>
              ))}
